...
namespace FtpCli.UnitTests
{
  public class FtpCli_TestData
  {
    ...
    { 
      _data.Server = server;
      Assert.Equal("randomServer", _data.Server);
    }
    ...
    { 
      _data.User = user;
      Assert.Equal("randomUser", _data.User);
    }

    [Theory]
    [InlineData(8080)]
    public void PortSetterAndGetter_SetsCorrectValue(int port) 
    { 
      _data.Port = port;
      Assert.Equal(8080, _data.Port);
    }
    ...
    { 
      ...
      Assert.Equal("SaltYourPasswordInReality", _data.Password);
    }

    [Theory]
    [InlineData("randomCommands arbitrary unexpected handled correctly")]
    public void CommandSetterAndGetter_SetsCorrectValue(string command) 
    { 
